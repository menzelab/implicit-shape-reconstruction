...
def _compute_bounding_box(mask):
    ...
    for axis in range(1, num_dims):
        ...
        proj = np.amax(mask, axis=tuple(max_over_axes))  # numpy expects a tuple of ints
...
def _crop_to_bounding_box(mask, bbox_min, bbox_max):
    ...
    # pyformat: enable
        ...
        raise ValueError(f'Unsupported dimensionality {num_dims}.')

    return cropmask


def _crop_to_bounding_box_exact(mask: np.ndarray, bbox_min: np.ndarray,
                                bbox_max: np.ndarray) -> np.ndarray:
    """Crop a 2D or 3D mask to given (inclusive) bounding box corners (without any
    additional padding).
    """
    num_dims = len(mask.shape)
    if num_dims == 2:
        return mask[bbox_min[0]:bbox_max[0] + 1,
                    bbox_min[1]:bbox_max[1] + 1]
    elif num_dims == 3:
        return mask[bbox_min[0]:bbox_max[0] + 1,
                    bbox_min[1]:bbox_max[1] + 1,
        ...
        raise ValueError('Unsupported mask dimensionality, expected 2 or 3, '
                         'instead got {}.'.format(num_dims))


def _pad_once(mask: np.ndarray, do_zero_padding: bool = True) -> np.ndarray:
    """Pad the mask once along each axis by either zeros or by replicating the
    nearest value. Padding occures only on positive axis direction.
    """
    if do_zero_padding:
        mask_padded = np.pad(mask, 1, 'constant', constant_values=0)
    else:
        mask_padded = np.pad(mask, 1, 'edge')
    # Remove the padding on the negative axis direction
    num_dims = len(mask.shape)
    if num_dims == 2:
        return mask_padded[1:, 1:]
    elif num_dims == 3:
        return mask_padded[1:, 1:, 1:]
    else:
        raise ValueError('Unsupported mask dimensionality, expected 2 or 3, '
                         'instead got {}.'.format(num_dims))
...
def compute_surface_distances(mask_gt,
                              mask_pred,
                              spacing_mm,
                              do_consider_boundary_voxels: bool = True):
    ...
    Args:
      ...
      do_consider_boundary_voxels: whether the foreground voxels at the mask's
        array boundary and surrounded by foreground voxels are considered as
        anatomy boundary or not. If the anatomy is not fully contained in the mask,
    ...
    # The original code was changed so that:
    # 1. the volume is cropped to larger size keep "true" boundary voxels at array boundary
    # 2. if boundary voxels are NOT to be considered, pad by replicating the edge (otherwise by 0)
    ...
    # possible processing subvolume. these are inclusive bbox corner coords
    ...
    # If we crop to the exact bounding box, we can't distunguish the foreground
    # voxels on the mask's array boundary anymore. Therefore, add padding to bbox
    # (clipped by the mask shape).
    # Clip each axis value independently (elementwise)
    bbox_min = np.maximum(bbox_min - 1, 0)
    bbox_max = np.minimum(bbox_max + 1, np.array(mask_gt.shape) - 1)

    # crop the processing subvolume.
    cropmask_gt = _crop_to_bounding_box_exact(mask_gt, bbox_min, bbox_max)
    cropmask_pred = _crop_to_bounding_box_exact(mask_pred, bbox_min, bbox_max)
    # If boundary voxels are to be considered, use zero-padding; repeat the edge
    # values otherwise.
    cropmask_gt = _pad_once(cropmask_gt, do_consider_boundary_voxels)
    cropmask_pred = _pad_once(cropmask_pred, do_consider_boundary_voxels)

    # compute the neighbour code (local binary pattern) for each voxel
    # the resulting arrays are spacially shifted by minus half a voxel in each
    # axis.
    # i.e. the points are located at the corners of the original voxels
    if do_consider_boundary_voxels:
        ...
        neighbour_code_map_gt = ndimage.correlate(
            cropmask_gt.astype(np.uint8), kernel, mode="nearest")
        neighbour_code_map_pred = ndimage.correlate(
            cropmask_pred.astype(np.uint8), kernel, mode="nearest")
...
def compute_average_surface_distance(surface_distances):
    ...
    if distances_gt_to_pred.size != 0:
        ...
        average_distance_gt_to_pred = np.nan
    if distances_pred_to_gt.size != 0:
        ...
        average_distance_pred_to_gt = np.nan
...
def compute_surface_dice_at_tolerance(surface_distances, tolerance_mm):
    ...
    surfel_areas_gt = surface_distances["surfel_areas_gt"]
    surfel_areas_pred = surface_distances["surfel_areas_pred"]
    if distances_gt_to_pred.size == 0 and distances_pred_to_gt.size == 0:
        return np.nan
